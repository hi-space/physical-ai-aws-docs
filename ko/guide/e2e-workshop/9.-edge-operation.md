...
1. AWS 콘솔 → CloudFormation → `IsaacLab-Stable-<userId>` 스택 → Outputs에서 `CodeServerUrl` 확인
2. 브라우저에서 CloudFront URL 접속 (예: `https://dXXXXXXXXXXXX.cloudfront.net`)
3. Password: Secrets Manager에 저장된 값 (DCV 비밀번호와 동일)
...
```bash
# 벤치마크 결과 확인 (인스턴스에서)
sudo grep -A5 "BENCHMARK RESULTS" /greengrass/v2/logs/com.workshop.${USER_ID}.benchmark.log
```
...
> **참고**: 서버가 정상 기동되었는지 상세 로그로 확인하려면 인스턴스에서 `docker logs groot-workshop-inference --tail 5`를 실행합니다. `Server is ready and listening on tcp://0.0.0.0:5555` 메시지가 출력되면 정상입니다.
...
{% hint style="info" %}
Setup 스크립트가 컴포넌트 이름에 `{USER_ID}`를 자동 삽입합니다. 로그 파일명도 이에 따라 `com.workshop.{USER_ID}.<컴포넌트>.log` 형식입니다.
{% endhint %}
