...
<summary>트러블슈팅: 배포 중 "No default subnet" 에러</summary>

아래와 같은 에러로 배포가 실패하면 특정 AZ에 Default Subnet이 없는 것입니다:
...
원인: AZ 탐색 Lambda가 probe용 `RunInstances`를 호출할 때 subnet을 지정하지 않으므로, 해당 AZ에 Default Subnet이 없으면 실패합니다. 리전에 새로운 AZ가 추가되었거나 누군가 Default Subnet을 삭제한 경우 발생할 수 있습니다.
...
nohup npx cdk deploy \
  -c userId=<본인이름> -c vpcCidr=<본인CIDR> \
  -c preferredAZ=0 \
  --require-approval never > deploy.log 2>&1 &
```

`preferredAZ=0`은 리전의 첫 번째 AZ를 선택합니다. AZ 이름을 직접 지정할 수도 있습니다 (예: `preferredAZ=us-east-1a`).
