...
`GrootFinetuneShared` 배포와 동시에 Batch용 CodeBuild(`groot-batch-train-build`)가 GR00T 컨테이너 이미지를 자동으로 빌드합니다. 약 15GB 크기의 이미지를 생성하므로 25~35분이 소요됩니다. SageMaker용 CodeBuild(`groot-sm-training-build`, `groot-sm-inference-build`)는 빈 프로젝트로 등록만 되며, 모듈 7에서 별도 트리거합니다.
