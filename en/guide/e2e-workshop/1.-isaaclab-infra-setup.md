...
This occurs because the AZ selector Lambda calls `RunInstances` without specifying a subnet. If an AZ lacks a Default Subnet (e.g., a newly added AZ in the region, or someone deleted the subnet), the probe fails for that AZ.
...
```bash
# Option 1: Create the missing Default Subnet
aws ec2 create-default-subnet --availability-zone us-east-1b

# Option 2: Specify AZ directly (skips Lambda probe entirely)
nohup npx cdk deploy \
